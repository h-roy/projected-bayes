...
from src.sampling import sample_projections_dataloader, vectorize_nn, sample_projections_ood_dataloader
from src.helper import set_seed
from src.data import get_cifar10, get_dataloaders, get_output_dim, get_ood_rand_dataset, get_ood_perturbation_dataset
import os
...
parser.add_argument("--num_ood_iterations", type=int, default=50)
...
parser.add_argument("--ood_type", type=str, choices=["random", "perturbation"], default="random")
parser.add_argument("--ood_variance", type=float, default=0.1)
...
if __name__ == "__main__":
    ...
    ood_batch_size = args.macro_batch_size
    ood_dataset_size = args.num_ood_examples
    data_shape = x_val.shape[1:]
    ood_type = args.ood_type
    ood_variance = args.ood_variance
    ood_n_samples_per_class = ood_dataset_size // output_dim

    if ood_type == "random":
        ood_loader = get_ood_rand_dataset(ood_batch_size, data_shape, output_dim, ood_dataset_size, seed)
    elif ood_type == "perturbation":
        ood_loader = get_ood_perturbation_dataset(dataset, ood_variance, ood_n_samples_per_class, ood_batch_size, seed)
    ...
    n_ood_iterations = args.num_ood_iterations
    ...
    posterior_samples, metrics = sample_projections_ood_dataloader(
                                                      model_fn_vec,
                                                      params_vec,
                                                      eps,
                                                      alpha,
